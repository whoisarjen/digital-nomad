...
**Recommendation:** Start with user submissions (IDEA-15 style) to seed the data, and explore Numbeo API access for automation once there's a budget.
...
**Modify** `apps/nomad/src/pages/cities/[slug].vue` (or `[slug]/index.vue` after IDEA-29 restructure)
...
## Dependencies
- IDEA-29: If city page is restructured to `[slug]/index.vue`, this component goes in that file.
...
- Price comparison ("A restaurant meal in Bangkok costs 70% less than in Lisbon") is a compelling feature that requires computing diffs client-side or via the compare endpoint (IDEA-08).
