...
## Dependencies
- **Favorite model** (already implemented, IDEA-20 equivalent is already done as `Favorite` exists in schema).
- IDEA-11 (Check-ins) is explicitly NOT a dependency in this revised plan — gating is done on Favorites only.

## Notes
- **Gating rationale change:** Gating on Favorites rather than check-ins is a deliberate simplification. It avoids blocking on IDEA-11 and is still a meaningful quality signal. Can be relaxed later to "favorited OR checked in" once IDEA-11 ships.
