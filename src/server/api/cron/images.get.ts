...
console.log(`Still missing images for ${cities.length} cities`)
  for (const { slug, name, country } of cities) {
    const data = await $fetch<{ results: Result[] }>(`REDACTED_IMAGE_API_URL?client_id=REDACTED_UNSPLASH_KEY&query=${[...name.split(' '), 'city', country].filter(option => option).join('%20')}`)
