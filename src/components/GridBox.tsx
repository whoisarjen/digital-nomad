...
export const GridBox = async ({ cityId, searchParams }: GridBoxProps) => {
    ...
    const mostCommonWeatherCode = getMostCommonWeatherCode(weathers)
    const avgTemperatureMax = mean(weathers.map(({ temperatureMax }) => temperatureMax))
            ...
            <AspectRatio ratio={16 / 9} className="bg-muted">
                ...
                <div className="text-white absolute inset-0 flex flex-col items-center justify-center bg-opacity-50">
                    ...
                    <div className="flex items-center gap-1 absolute bottom-0 left-0 p-3 text-xs">
                        ...
                        {!isNil(avgTemperatureMax) &&
                            <>
                                <div>|</div>
                                <div className="flex items-center gap-1">
                                    <IconWeather weatherCode={mostCommonWeatherCode} size={18} />
                                    <div className="flex flex-col">
                                        <span>{round(avgTemperatureMax, 1)}°C</span>
